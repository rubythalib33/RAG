...
def chat_with_ollama(messages):
    ...
    data = {
        "model": "llama3.1:latest",  # Replace with the actual model you are using from Ollama
        "prompt": messages,
        "stream": False,
        "options": {
            "num_ctx": 4096,
...
async def chat_with_session(session_id: int, text: str):
    session_history = [{"role": "system", "content": CHATBOT_PREPROMPT}]
    ...
    combined_context = "Knowledge Context:\n"+knowledge_texts+"\n Chat History Context:\n"+previous_chat_texts
    ...
    response_content = chat_with_ollama(session_history).replace('#','').replace('*','')
